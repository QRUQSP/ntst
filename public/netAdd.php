...
// -------
//
function qruqsp_ntst_netAdd(&$ciniki) {
    //
    ...
    $rc = qruqsp_ntst_checkAccess($ciniki, $args['tnid'], 'qruqsp.ntst.netAdd');
    if( $rc['stat'] != 'ok' ) {
        return $rc;
    }
    ...
    if( trim($start_utc) != '' ) {
        ...
        if( $ts !== false && $ts > 0 ) {
            ...
            return array('stat'=>'fail', 'err'=>array('code'=>'qruqsp.ntst.9', 'msg'=>'Invalid start date & time'));
        }
    } else {
        $args['start_utc'] = '';
    }
    ...
    if( trim($end_utc) != '' ) {
        ...
        if( $ts !== false && $ts > 0 ) {
            ...
            return array('stat'=>'fail', 'err'=>array('code'=>'qruqsp.ntst.35', 'msg'=>'Invalid end date & time'));
        }
    } else {
        $args['end_utc'] = '';
    }
    ...
    $rc = ciniki_core_objectAdd($ciniki, $args['tnid'], 'qruqsp.ntst.net', $args, 0x04);
    if( $rc['stat'] != 'ok' ) {
    ...
    ciniki_core_hookExec($ciniki, $args['tnid'], 'ciniki', 'web', 'indexObject', array('object'=>'qruqsp.ntst.net', 'object_id'=>$net_id));
